...
static int npc_create(lua_State *s)
{
    ...
    const int id = luaL_checkinteger(s, 2);
    const int gender = luaL_checkinteger(s, 3);
    const int x = luaL_checkinteger(s, 4);
    const int y = luaL_checkinteger(s, 5);
...
{
    const int x = luaL_checkinteger(s, 1);
    const int y = luaL_checkinteger(s, 2);
    const int width = luaL_checkinteger(s, 3);
    const int height = luaL_checkinteger(s, 4);
    luaL_checktype(s, 5, LUA_TFUNCTION);
    const int id = luaL_checkinteger(s, 6);
...
{
    const int id = luaL_checkinteger(s, 1);
    ...
    {
        // positional mode
        int x = luaL_checkinteger(s, 2);
        int y = luaL_checkinteger(s, 3);
        MapComposite *m = checkCurrentMap(s);
...
static int item_drop(lua_State *s)
{
    ...
    const int number = luaL_optinteger(s, 4, 1);
...
{
    int min = luaL_checkinteger(s, 1);
    int max = luaL_checkinteger(s, 2);
    int defaultValue = luaL_optinteger(s, 3, min);
...
static int entity_equip_slot(lua_State *s)
{
    ...
    int inventorySlot = luaL_checkinteger(s, 2);
...
{
    Entity *ch = checkCharacter(s, 1);
    int equipmentSlot = luaL_checkinteger(s, 2);
...
static int entity_set_ability_cooldown(lua_State *s)
{
    ...
    const int ticks = luaL_checkinteger(s, 3);
...
{
    Entity *c = checkCharacter(s, 1);
    const int ticks = luaL_checkinteger(s, 2);
...
static int entity_look_at(lua_State *s)
{
    ...
    if (lua_gettop(s) > 2)
    {
        otherPoint.x = luaL_checkinteger(s, 2);
        otherPoint.y = luaL_checkinteger(s, 3);
    }
    else
    {
...
static int entity_heal(lua_State *s)
{
    ...
    if (lua_gettop(s) == 1) // when there is only one argument
        ...
        being->getComponent<BeingComponent>()->heal(*being, luaL_checkinteger(s, 2));
...
{
    Entity *being = checkBeing(s, 1);
    BeingAction act = static_cast<BeingAction>(luaL_checkinteger(s, 2));
...
{
    Entity *being = checkBeing(s, 1);
    BeingDirection dir = static_cast<BeingDirection>(luaL_checkinteger(s, 2));
...
{
    Entity *character = checkCharacter(s, 1);
    int x = luaL_checkinteger(s, 3);
    int y = luaL_checkinteger(s, 4);
...
static int entity_apply_attribute_modifier(lua_State *s)
{
    ...
    auto *attribute = checkAttribute(s, 2);
    double value    = luaL_checknumber(s, 3);
    int layer       = luaL_checkinteger(s, 4);
    int duration    = luaL_optinteger(s, 5, 0);
    int effectId    = luaL_optinteger(s, 6, 0);
...
static int entity_remove_attribute_modifier(lua_State *s)
{
    ...
    double value    = luaL_checknumber(s, 3);
    int layer       = luaL_checkinteger(s, 4);
    int effectId    = luaL_optinteger(s, 5, 0);
...
{
    Entity *being = checkCharacter(s, 1);
    int points = luaL_checkinteger(s, 2);
    auto *characterComponent = being->getComponent<CharacterComponent>();
...
{
    Entity *c = checkCharacter(s, 1);
    const int color = luaL_checkinteger(s, 2);
...
{
    Entity *c = checkCharacter(s, 1);
    const int style = luaL_checkinteger(s, 2);
...
{
    Entity *b = checkBeing(s, 1);
    const int ability = luaL_checkinteger(s, 2);
...
static int entity_use_ability(lua_State *s)
{
    ...
    {
        const int x = luaL_checkinteger(s, 3);
        const int y = luaL_checkinteger(s, 4);
...
static int entity_apply_status(lua_State *s)
{
    Entity *being = checkBeing(s, 1);
    const int id = luaL_checkinteger(s, 2);
    const int time = luaL_checkinteger(s, 3);
...
{
    const int startX = luaL_checkinteger(s, 1);
    const int startY = luaL_checkinteger(s, 2);
    const int destX = luaL_checkinteger(s, 3);
    const int destY = luaL_checkinteger(s, 4);
    unsigned maxRange = luaL_checkinteger(s, 5);
...
static int log(lua_State *s)
{
    ...
    const int loglevel = luaL_checkinteger(s, 1);
    luaL_argcheck(s,
...
static int get_beings_in_circle(lua_State *s)
{
    ...
    if (lua_isuserdata(s, 1))
    {
        ...
        x = pos.x;
        y = pos.y;
        r = luaL_checkinteger(s, 2);
    }
    else
    {
        x = luaL_checkinteger(s, 1);
        y = luaL_checkinteger(s, 2);
        r = luaL_checkinteger(s, 3);
    }
...
static int get_beings_in_rectangle(lua_State *s)
{
    const int x = luaL_checkinteger(s, 1);
    const int y = luaL_checkinteger(s, 2);
    const int w = luaL_checkinteger(s, 3);
    const int h = luaL_checkinteger(s, 4);
...
static int get_distance(lua_State *s)
{
    ...
    {
        x1 = luaL_checkinteger(s, 1);
        y1 = luaL_checkinteger(s, 2);
        x2 = luaL_checkinteger(s, 3);
        y2 = luaL_checkinteger(s, 4);
    }
    const int dx = x1 - x2;
    const int dy = y1 - y2;
